/**
(Geography: estimate areas) Use the GPS locations for Atlanta, Georgia; Orlando, Florida; Savannah, Georgia; and Charlotte, North Carolina in the figure in Section
...
public class Exercise4_03 {
    public static void main(String[] args) {
        ...
        double distanceChaAtl = RADIUS_OF_THE_EARTH * Math.acos(Math.sin(Math.toRadians(chaX)) * Math.sin(Math.toRadians(atlX)) + Math.cos(Math.toRadians(chaX)) * Math.cos(Math.toRadians(atlX)) *
Math.cos(Math.toRadians(chaY - atlY)));
        double distanceAtlOrl = RADIUS_OF_THE_EARTH * Math.acos(Math.sin(Math.toRadians(atlX)) * Math.sin(Math.toRadians(orlX)) + Math.cos(Math.toRadians(atlX)) * Math.cos(Math.toRadians(orlX)) * Math.cos(Math.toRadians(atlY - orlY)));
        double distanceOrlSav = RADIUS_OF_THE_EARTH * Math.acos(Math.sin(Math.toRadians(orlX)) * Math.sin(Math.toRadians(savX)) +
Math.cos(Math.toRadians(orlX)) * Math.cos(Math.toRadians(savX)) * Math.cos(Math.toRadians(orlY - savY)));
        double distanceSavCha = RADIUS_OF_THE_EARTH * Math.acos(Math.sin(Math.toRadians(savX)) * Math.sin(Math.toRadians(chaX)) +
Math.cos(Math.toRadians(savX)) * Math.cos(Math.toRadians(chaX)) * Math.cos(Math.toRadians(savY - chaY)));
        double distanceChaOrl = RADIUS_OF_THE_EARTH * Math.acos(Math.sin(Math.toRadians(chaX)) * Math.sin(Math.toRadians(orlX)) +
